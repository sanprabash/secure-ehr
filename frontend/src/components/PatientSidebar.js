...
function PatientSidebar() {
  ...
  const initials = user ? `${user.firstName[0]}${user.lastName[0]}` : 'PT';
  ...
  return (
    <div style={styles.sidebar}>
      ...
      <nav style={styles.nav}>
        ...
        <div style={isActive('/patient/dashboard') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
          onClick={() => navigate('/patient/dashboard')}>
          <span style={styles.navIcon}>⊞</span> Dashboard
        </div>

        <div style={isActive('/patient/records') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
          onClick={() => navigate('/patient/records')}>
          <span style={styles.navIcon}>📋</span> Medical Records
        </div>

        <div style={isActive('/patient/upload') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
          onClick={() => navigate('/patient/upload')}>
          <span style={styles.navIcon}>⬆</span> Upload Record
        </div>

        <div style={isActive('/patient/consent') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
          onClick={() => navigate('/patient/consent')}>
          <span style={styles.navIcon}>🔑</span> Consent Management
        </div>

        <div style={isActive('/patient/notifications') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
        ...
        <div style={isActive('/patient/profile') ? {...styles.navItem, ...styles.navItemActive} : styles.navItem}
